...
/** Tile de estadística: label + valor con tipografía condensada y tono opcional (e.g. text-danger). `title` es un tooltip nativo explicando qué mide el KPI. */
export function StatTile({ label, value, sub, tone, compact = false, title }: { label: string; value: string; sub?: ReactNode; tone?: string; compact?: boolean; title?: string }) {
  return (
    <div title={title} className={cn('w-fit min-w-[132px] rounded-xl border border-border bg-bg-elevated', compact ? 'px-2.5 py-2' : 'p-3')}>
